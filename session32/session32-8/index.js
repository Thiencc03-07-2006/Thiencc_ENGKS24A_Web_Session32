  ...
  const htmls = quest.map((student, index) => {
    return `
              <tr style="border-bottom: 1px solid gainsboro;display:flex;justify-content: space-between">
                <td >${student.text}</td>
                <td>
                  <button style="border:none" onclick="handleEdit(${index})">Sửa</button>
                  <button style="border:none" onclick="handleDelete(${index})">Xóa</button>
                </td>
              </tr>
          `;
  });
...
function handleEdit(index) {
  const confrimEdit = prompt("Chỉnh sửa công việc:");
  if (confrimEdit !== "") {
    // Tiến hành xóa
    quest[index] = { text: confrimEdit };
